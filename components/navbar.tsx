...
export default function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      // Update scroll progress for name transition
  ...
  // Calculate scroll progress for Portfolio fade based on about section
  const calculateScrollProgress = () => {
    const aboutSection = document.getElementById('about')
    if (!aboutSection) return 0
    
    const aboutTop = aboutSection.offsetTop
    const triggerPoint = aboutTop - 200 // Start transition 200px before about section
    const transitionRange = 300 // Complete transition over 300px
    
    return Math.min(Math.max((window.scrollY - triggerPoint) / transitionRange, 0), 1)
  }
        ...
        <div className="flex h-16 items-center justify-between">
          <div className="flex items-center flex-shrink-0 relative">
            <Link
              ref={portfolioRef}
              href="/"
              className="text-xl font-bold tracking-tighter transition-all duration-300 absolute"
              style={{
